<section class="mt-4">
    <h1 class=""><?= $author->first_name.mb_strtoupper($author->last_name) ; ?><span class="small ms-2 "><sup class="badge rounded-pill bg-info pb-1 pt-0 px-2"><?= numberFormat(count($games)) ; ?></sup></span></h1>
    <?php 
        if (isset($toolBar)) {
            echo $toolBar ; 
        } 
                    ...
                    <tbody>
                        <?php if ($games) :
                            ...
                            <tr>
                                ...
                                <td>
                                    <?= $game->is_author ? '<i class="fa-solid fa-pen-alt fa-fw text-primary"></i>' : '' ; ?>
                                    <?= $game->is_illustrator ? '<i class="fa-solid fa-paint-brush fa-fw text-primary"></i>' : '' ; ?>
                                </td>
                            </tr>
                            <?php endforeach ; 
                        else : ?>
                            <tr><td colspan="5" class="text-danger fw-bold"><i class="fa-solid fa-exclamation-circle me-1"></i>Aucun jeu</td></tr>
                        <?php endif ;?>
                    </tbody>
                </table>
            </div>
